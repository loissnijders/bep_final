...
def register_callbacks_4(app):
                        ...
                        children=[
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    #             label=f'Number of Categories for {feature}',
                                    #             labelPosition='top',
                                    #             size=120,
                                    #         ),
                                    ...
                                    dcc.RadioItems(
                                        ...
                                        value='frequency',  # Default option
                                        labelStyle={'display': 'block'}
                                    )
                                ]
                            ),
    ...
    @app.callback(
        Output({'type': 'dynamic-choice', 'index': dash.dependencies.MATCH},'children'),
        Input({'type': 'categories-subset', 'index': dash.dependencies.MATCH}, 'value')
    )
    def update_dynamic_input(subset_type):
        if subset_type in ['most_occurring', 'least_occurring']:
            # Display numeric input for most/least occurring
            return html.Div([
                html.Label('Enter number of items:'),
                dcc.Input(
                    id={'type': 'numeric-input', 'index': dash.dependencies.MATCH},
                    min=1,
                    # max=num_distinct_classes,
                    # value=num_distinct_classes,
                    label=f'Number of Categories for {dash.dependencies.MATCH}',
                    labelPosition='top',
                    size=120,
                )
            ])
            ...
            return html.Div([
                ...
                dcc.Dropdown(
                    id={'type': 'manual-subset-dropdown', 'index': dash.dependencies.MATCH},
                    # options=[{'label': cat, 'value': cat} for cat in dataframe[dash.dependencies.MATCH],
                    multi=True,
                    placeholder='Select categories manually',
                    style={'marginTop': '10px'}
                )
            ...
            return html.Div()  # Empty div if no selection matches
